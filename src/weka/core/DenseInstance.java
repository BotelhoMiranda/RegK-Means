...
public class DenseInstance extends AbstractInstance {
  ...
  private double minDistanceFromCentroid;
  
  protected ArrayList<String> neighborhood;
...
@Override
public void setNeighborhood(String neighbors) {
	
	neighborhood = new ArrayList<String>();
	String neibinhos = neighbors;
	String[] quebrados = neibinhos.split("_");
	for (int i = 0; i < quebrados.length; i++){
		quebrados[i] = quebrados[i].replace("[", "");
	    quebrados[i] = quebrados[i].replace("]", "");
	    int quebrada = Integer.parseInt(quebrados[i]);
	    quebrada = quebrada - 1;
	    quebrados[i] = Integer.toString(quebrada);
		neighborhood.add(quebrados[i]);
	}
...
@Override
public boolean isNeighborhoord(ArrayList<String> myNeighbors,String instance){
	
	if(myNeighbors.isEmpty())
		return true;
	
//	for (String it : neighborhood) {
//		System.out.println("peraiiiii");
//		System.out.println(it);
//		System.out.println("///////////////////////////////////");
		if(myNeighbors.contains(instance))
			return true;
//	}
	return false;
}
...
@Override
public void distanceFromCentroide(double minDistance) {
	// TODO Auto-generated method stub
	minDistanceFromCentroid = minDistance;
}

@Override
public double getDistanceFromCentroide() {
	// TODO Auto-generated method stub
	return minDistanceFromCentroid;
}
}
